# GrammarApp.py (Adaptado para GrammarLogicNLTK)

import tkinter as tk
import json
from tkinter import filedialog, ttk, messagebox, scrolledtext
# Importar la nueva clase lógica
from GrammarLogic import GrammarLogicNLTK
from Constants import MAX_GENERATED_STRINGS # EPSILON ya no se usa tanto

# nltk Tree (opcional, solo si quieres dibujarlo gráficamente)
# from nltk import Tree
# from nltk.draw.tree import TreeView # Requiere dependencias externas como ghostscript

class GrammarApp:
    def __init__(self, master):
        self.master = master
        master.title("Generador de Gramáticas (NLTK)") # Título actualizado
    ...
    def validate_string_action(self):
        ...
        try:
            belongs, parse_trees = self.logic.validate_string(input_string)
    ...
    def show_derivation_tree_window(self, input_string, tree_output, tree_count):
        ...
        deriv_win.geometry("600x450") # Ajustar tamaño si es necesario

        frame = ttk.Frame(deriv_win, padding="10")
        frame.pack(expand=True, fill=tk.BOTH)

        lbl = ttk.Label(frame, text="Árbol de Derivación (Formato NLTK):", font=('Helvetica', 11, 'bold'))
        lbl.pack(pady=(0, 5))
    ...
    def load_grammar_action(self):
        ...
        try:
            ...
            # Opcional: validar automáticamente
            # self.validate_grammar_type_action()
